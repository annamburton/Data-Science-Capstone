...
st.set_page_config(page_title="Churn RF Dashboard", layout="wide")
...
def prepare_for_rf(raw_row: pd.DataFrame, preprocess: dict):
    X = raw_row.copy()
...
model_path = st.sidebar.text_input(
    ...
    value="rf_model.pkl",
    help="Path to the saved Random Forest model."
)
preprocess_path = st.sidebar.text_input(
    "Preprocess artifact path",
    value="rf_preprocess.pkl",
...
# ---------- Header ----------
st.title("Customer Churn Random Forest Dashboard")
st.caption("Random Forest model for churn risk prediction")
...
with left:
    ...
    st.write(
        "This dashboard presents the Random Forest model for churn prediction. "
        "It improves on the baseline by capturing non-linear relationships "
    )
...
st.write(
    ...
    "Random Forest typically improves recall over logistic regression by better capturing "
    "complex, non-linear churn signals."
)
...
with c2:
    ...
    if model is not None and preprocess is not None:
        try:
            X_row = prepare_for_rf(selected_row, preprocess)
            ...
            ax.barh(["Churn probability"], [prob], color="steelblue")
            ax.set_xlim(0, 1)
            ax.set_xlabel("Probability")
            ax.set_title("RF risk score")
        ...
        st.info(
            "Model artifacts are not loaded yet. Once rf_model.pkl and rf_preprocess.pkl "
...
st.write(
    "Random Forest captures complex patterns between engagement, pricing, and subscription "
    "signals that logistic regression may miss. The feature importance chart highlights which "
    "factors influence churn risk most in this ensemble model. Use this alongside the baseline "
    "to understand what the additional model complexity is buying you."
)

# ---------- Footer ----------
st.markdown("---")
st.write(
    "Random Forest dashboard — part of the KKBox churn prediction project."
)
